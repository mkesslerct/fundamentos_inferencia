...
## Distribuciones t de Student y F de Snedecor
...
$$ f_{t_k}(t)\propto \frac 1 {(1+t^2/k)^{(k+1)/2}},\quad -\infty<t<\infty,$$
Por las propiedades de una función de densidad se puede demostrar que la constante de proporcionalidad es
$$K=\frac {\Gamma(\frac{k+1} 2)}{\Gamma(\frac k 2)}\frac 1 {\sqrt{k\pi}},$$
...
::: {.proof}
Vamos a demostrar que $T=\frac{\bar{X}-\mu}{S/\sqrt{n}}$ tiene por densidad la expresión dada en la @eq-densstud. Para ello, introducimos las dos variables $U = (\bar{X}_n - \mu)/\sqrt{\sigma/n}$ que sigue una distribución normal estándar y $V =  (n-1)S_n^2/\sigma^2$ que sigue una distribución ji-cuadrado con $n-1$ grados de libertad. Tenemos que $T = U/\sqrt{V/(n-1)}$. Como son independientes, la densidad conjunta de $U$ y $V$ es el producto de sus densidades:
$$ f_{U, V}(u, v) = \frac 1 {(2\pi)^2} e^{-u^2/2}\frac 1 {\Gamma(\frac{n - 1} 2) 2^{(n -  1)/2}} v^{(p/2) - 1} e^{- v/2}, \quad -\infty<u<\infty,\ 0<v<\infty.$$
:::
La distribución de $T$ depende por lo tanto del tamaño $n$ de la muestra, a través de los llamados ``grados de libertad''. Se puede demostrar que la densidad $F_{t_{k}}$ de la distribución t de Student con $k$ grados de libertad admite la siguiente expresión:

La distribución t tiene colas más pesadas que la distribución Normal, lo que es intuitivamente natural puesto que, al obtenerse $T$ sustituyendo $\sigma$ por $S$, el denominador de $T$ presenta ahora también variabilidad. Esta variabilidad en el denominador resulta en que $T$ puede tomar con más probabilidad valores más extremos. Sin embargo, si los grados de libertad aumentan, la variabilidad de $S$ disminuye, y la distribución t de Student asociada se parece más a una Normal. 
