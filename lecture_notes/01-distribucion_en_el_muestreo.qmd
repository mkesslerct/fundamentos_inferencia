...
::: {.callout-important}
## Distribución t de Student
La distribución t de Student con $k$ grados de libertad está definido por su función de densidad:
$$ f_{t_k}(t)= \frac {\Gamma(\frac{k+1} 2)}{\Gamma(\frac k 2)}\frac 1 {\sqrt{k\pi}} \frac 1 {(1+t^2/k)^{(k+1)/2}},\quad -\infty<t<\infty,$$
...
[^1]: La función Gamma  tiene la expresión siguiente: para cualquier real $\alpha>0$, $\Gamma(\alpha)=\int_{0}^\infty t^{\alpha-1}e^{-t}dt.$
    ...
    $$ f_{\alpha, \beta}(x) = \frac 1 {\Gamma(\alpha)\beta^\alpha} x^{\alpha - 1} e^{-x/\beta}, \quad y>0$$ {#eq-gammadens}
...
Sea $\Phi$ la aplicación que transforma $(Z, Y)$  en $(T, Y)$, $\Phi(z, y) = (z/\sqrt{y/(n - 1)}, y)$. Se trata de un difeormorphismo de $\mathbb{R} \times\mathbb{R}^+$ a  $\mathbb{R} \times\mathbb{R}^+$. Deducimos que la densidad conjunta de $(T, Y)$ es 
...
Esta distribución se llama F de Snedecor con $p_1$ y $p_2$ grados de libertad y se denota $F_{p_1, p_2}$.
:::
...
Consideremos dos variables aleatorias normales $X$ e $Y$ con varianzas $\sigma_2^X$ y $\sigma_2^Y$, así como $X_1, \ldots, X_{n_1}$ y $Y_1, \ldots, Y_{n_2}$ dos muestras aleatorias simples de $X$ y $Y$ respectivamente. Por la @prp-F-Snedecor sabemos que
$$\frac{(n_1-1)S_{n_1}^X/\sigma_X^2}{(n_2-1)S_{n_2}^Y/\sigma_Y^2}\sim F_{n_1-1, n_2-1}.$$
