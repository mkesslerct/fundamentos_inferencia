...
-   Dibujo en la densidad del estadístico
    ...
    #| fig-cap: Región central con un área $\alpha$ en la densidad de una Normal estándar. 
    #| warning: false
    #| fig-width: 10
    #| fig-height: 7
    ...
    x <- seq(-5, 5, by = 0.01)
    phi <- dnorm(x)
    cent <- between(x, qnorm(alpha/2), qnorm(1 - alpha/2))  
    xzalf <- x[cent]
    phizalf <- phi[cent]
    vertices.x <- c(xzalf[1], xzalf, xzalf[length(xzalf)])
    vertices.y <- c(0, phizalf, 0)
    ...
    axis(
        ...
        at=c(-5, qnorm(alpha/2), 0, qnorm(1-alpha/2), 5)
    )
    ```
...
Para ilustrar esta interpretación, he simulado 20 veces el proceso de
extraer una muestra de tamaño 4 de una distribución
$X\sim \mathcal{N}(\mu_X,1)$. He representado en la Figura
@fig-ics en el eje Ox
...
el correspondiente a la muestra número 13 que no lo hace: este intervalo
es erróneo, y esta muestra forma parte del 5% de las muestras "malas",
es decir las que proporcionan intervalos equivocados.

![Los intervalos de confianza al 95% correspondientes a 20 muestras de
tamaño 4. La media que se busca estimar es $\mu=2$.](figures/ics.png){#fig-ics
width="8cm"}
...
:   La construcción del intervalo de confianza está basada en la
    ...
    recurrir al Teorema Central del Límite, ver
    [\[tcl\]](#tcl){reference-type="ref" reference="tcl"}, y sabemos que
    $$\mbox{aproximadamente, }\frac {\bar{X}-\mu} {\sigma/\sqrt{n}}\sim \mathcal{N}(0,1),$$
...
```{r}
    ...
    #| fig-cap: Región central con un área $\alpha$ en la densidad de una t de Student. 
    #| warning: false
    #| fig-width: 10
    #| fig-height: 7
    library(dplyr)
    ...
    x <- seq(-5, 5, by = 0.01)
    phi <- dt(x, df = 10)
    cent <- between(x, qt(alpha/2, 10), qt(1 - alpha/2, 10))  
    xtalf <- x[cent]
    phitalf <- phi[cent]
    vertices.x <- c(xtalf[1], xtalf, xtalf[length(xtalf)])
    vertices.y <- c(0, phitalf, 0)
    plot(x, phi, type="l", xlab="", ylab="", axes=FALSE)
    polygon(vertices.x, vertices.y, density=10)
    axis(
        1,
